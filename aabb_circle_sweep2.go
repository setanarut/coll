...
func AABBCircleSweep2(box *AABB, circle *Circle, boxVel, circleVel v.Vec) bool {
	...
	// --- ERKEN ÇIKIŞ 1: Statik Çarpışma Kontrolü (t=0) ---
	// Var olan AABBCircleOverlap metodunu kullan. Eğer başlangıçta çakışma varsa, hemen TRUE dön.
	...
	// --- ADIM 1: Genişletilmiş AABB Yüzey Kontrolü (Slab Method) ---
	...
	if V.Y != 0.0 {
		...
		if circle.Pos.Y < expandedMin.Y || circle.Pos.Y > expandedMax.Y {
			return false // ERKEN ÇIKIŞ 3
		}
	}

	// ERKEN ÇIKIŞ 4: Slab metodu çarpışmayı dışladı.
	if t_enter > t_exit || t_enter > 1.0 {
		return false
	}
	// NOT: t_enter < 0.0 kontrolü artık gerekli değil çünkü AABBCircleOverlap bunu zaten halletti.

	// --- ADIM 2: Köşe Noktası Kontrolleri ---
	...
	for _, corner := range corners {
		...
		// Statik çakışma (t=0) veya hareket yok durumu başlangıçta kontrol edildi.
		if A < 0.000001 {
			continue
		}
		...
		// Sadece ileri yöndeki çarpışmalarla ilgileniyoruz (t > 0).
		if t_col > 0.0 && t_col <= 1.0 {
			min_t_corner = min(min_t_corner, t_col)
		}
	}

	// --- ADIM 3: Sonuç Birleştirme ---

	final_t_col := min(t_enter, min_t_corner)

	// En erken çarpışma zamanı (0, 1] aralığında ise çarpışma vardır.
